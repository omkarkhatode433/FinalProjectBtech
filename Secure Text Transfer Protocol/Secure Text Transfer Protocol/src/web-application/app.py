...
@app.route('/Download-from-cloud/')
def DownloadFiles():
	items =FileAccess.list_files()
	if not items:
		return render_template('file-list.html',msg='Files Not Found!!!')
	else:
		return render_template('cloud-file-list.html',msg='',itr=0,length=len(items),list=items)
...
def download_files():
	if request.method == 'POST':
		...
		FileDownload.download_file(file_id,file_name)
...
def cloud():
	...
	FileUploadCloud.upload_to_drive(file_path)
	root.destroy()
	return render_template('file-list.html',msg='File Uploaded to cloud Successfully!!!')
...
def upload_file():
	if request.method == 'POST':
		...
		if file:
			...
			print(file.filename)
			# FileUploadCloud.upload_to_drive('file.txt')
